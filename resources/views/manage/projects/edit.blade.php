                        ...
                        <div class="translatable-content">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="exampleInputName1" name="title"
                                    placeholder="Title" value="{{ $project->langs->firstWhere('lang',app()->getLocale())->title }}" />
                                @error('title')
                            ...
                            <div class="form-group">
                                ...
                                <input type="hidden" name="description_lang"
                                    value='{"az":"{{ $project->langs->firstWhere('lang', 'az')->description }}","ru":"{{ $project->langs->firstWhere('lang', 'ru')->description }}","en":"{{ $project->langs->firstWhere('lang', 'en')->description }}"}'>
                                <textarea name="description" class="form-control" rows="5">{{ $project->langs->firstWhere('lang',app()->getLocale())->description }}</textarea>
                                @error('description')
                                    <span class="text-danger mt-2 d-inline-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="upload-box">
                                @foreach ($project->images as $image)
                                    <div class="image-box">
                                        <img src="{{ asset('uploads/projects/' . $image->image) }}" width="200" alt="">
                        ...
                        <div class="form-group">
                            ...
                            <select name="category" id="" class="form-control">
                                @foreach ($categories as $category)
                                    <option {{ $category->id == $project->category_id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->langs->firstWhere('lang',app()->getLocale())->name }}</option>
                                @endforeach
                            </select>
                        </div>
