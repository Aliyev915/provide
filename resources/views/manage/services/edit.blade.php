...
$en = '';
$ru = '';
foreach ($service->langs->firstWhere('lang', 'az')->suggestions as $key=>$suggestion) {
    if($key < count($service->langs->firstWhere('lang', 'az')->suggestions)-1){
        $az .= $suggestion.',';
    }else{
        $az .= $suggestion;
    }
}
foreach ($service->langs->firstWhere('lang', 'en')->suggestions as $key=>$suggestion) {
    if($key < count($service->langs->firstWhere('lang', 'en')->suggestions)-1){
        $en .= $suggestion.',';
    }else{
        $en .= $suggestion;
    }
}
foreach ($service->langs->firstWhere('lang', 'ru')->suggestions as $key=>$suggestion) {
    if($key < count($service->langs->firstWhere('lang', 'ru')->suggestions)-1){
        $ru .= $suggestion.',';
    }else{
        $ru .= $suggestion;
    }
                    ...
                    <form class="forms-sample" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="translatable-content">
                            ...
                            <div class="form-group">
                                ...
                                <input type="hidden" name="suggestion_lang"
                                    value='{"az":"{{ $az }}","ru":"{{ $ru }}","en":"{{ $en }}"}'>
                                <input type="text" class="form-control" id="exampleInputName1" name="suggestions"
                                    placeholder="suggestion1, suggestion2,..."
                                    value="{{ app()->isLocale('az') ? $az : app()->isLocale('en') ? $en : $ru  }}" />
                                @error('suggestions')
                                    <span class="text-danger mt-2 d-inline-block">{{ $message }}</span>
                                @enderror
                            </div>
